...
export async function registerUser(data) {
  const payload = typeof data === 'object' ? data : { name: arguments[0], email: arguments[1], password: arguments[2], phone: arguments[3] };
...
export async function updateUserAddress(addressId, addressData) {
  return request(`/api/auth/addresses/${encodeURIComponent(addressId)}`, {
    method: 'PUT',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify(addressData)
  });
...
export async function setDefaultUserAddress(addressId) {
  return request(`/api/auth/addresses/${encodeURIComponent(addressId)}/default`, {
    method: 'PATCH'
  });
}
...
export async function updateCategory(idOrSlug, categoryData) {
  return request(`/api/categories/${encodeURIComponent(idOrSlug)}`, {
    method: 'PUT',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify(categoryData)
...
export async function fetchBrandById(idOrSlug) {
  return request(`/api/brands/${encodeURIComponent(idOrSlug)}`);
}
...
export async function updateBrand(idOrSlug, brandData) {
  return request(`/api/brands/${encodeURIComponent(idOrSlug)}`, {
    method: 'PUT',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify(brandData)
...
export async function fetchAdminCustomers(params = {}) {
  ...
  const queryString = query.toString();
  return request(queryString ? `/api/admin/customers?${queryString}` : '/api/admin/customers');
}

export async function fetchAdminCustomerById(id) {
  return request(`/api/admin/customers/${encodeURIComponent(id)}`);
}
...
export async function fetchAdminOrders(params = {}) {
  ...
  const queryString = query.toString();
  return request(queryString ? `/api/admin/orders?${queryString}` : '/api/admin/orders');
}
...
export async function updateAdminProductStock(id, stock) {
  return request(`/api/admin/products/${encodeURIComponent(id)}/stock`, {
    method: 'PATCH',
    headers: { 'Content-Type': 'application/json' },
...
export async function fetchAdminReviews(params = {}) {
  ...
  if (params.search) query.append('search', params.search);
  const queryString = query.toString();
  return request(queryString ? `/api/admin/reviews?${queryString}` : '/api/admin/reviews');
}
...
export async function deleteAdminReview(id) {
  return request(`/api/admin/reviews/${encodeURIComponent(id)}`, {
    method: 'DELETE'
  });
}
 ...
 * 7. Image Upload API
 */
export async function uploadImageFile(file, folder = 'saravana-women-dresses/products') {
